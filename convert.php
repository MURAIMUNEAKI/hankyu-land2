...
あなたは旅行業務の専門家です。与えられたテキストから重要な情報だけを正確に抽出・要約してください。

【絶対ルール】
- 必ず日本語で回答すること
- 「?」「？」だけを出力してはいけない
- 意味不明な記号のみの出力は禁止
- 要約すべき内容がない場合は、何も出力せず空文字を返すこと
EOT;

    $userPrompt = <<<EOT
以下の旅行業務テキストを要約してください。

【必ず記載する項目】
- 「アレルギー」「車椅子での参加」「医療機器」など具体的な支援ニーズ
- お客からの要望（RQ）
- 特別依頼：「ハネムーン」「結婚記念日」「誕生日」

【削除する項目（要約に含めない）】
- 一般的な持参薬案内
- 確定（HK）や変更履歴（例：TWN SGL→）
- 金銭/保険/旅券/連絡先変更/JR/社内進行に関する記述
- 「☆☆ダミー記録です☆☆」のようなダミーデータ表記
- ルーティン的な確認事項

【出力形式】
- 簡潔にまとめる（最大80文字以内）
- 重要な情報のみを抽出する
- 必ず完結した文章で出力する（途中で切れない）
- 記載すべき重要情報がない場合は、何も出力しない（空文字列を返す）
- 「?」や「（空文字列）」などの無意味な出力は絶対にしない
...
EOT;
    ...
    for ($attempt = 0; $attempt <= $maxRetries; $attempt++) {
        ...
        // 「なし」「特になし」「該当なし」等の出力を空にする
        if (preg_match('/^[\s]*(特に)?[（(]?(なし|該当なし|記載なし|情報なし)[）)]?[\s]*[。.]?[\s]*$/u', $text)) {
            $text = '';
        }
